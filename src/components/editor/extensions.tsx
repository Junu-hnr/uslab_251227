...
export const createSuggestionItemsWithUpload = (
    ...
    const getRange = (editor: any, range: any) => {
        if (range) return range;
        const { from, to } = editor.state.selection;
        const textBefore = editor.state.doc.textBetween(Math.max(0, from - 20), from);
        const lastSlash = textBefore.lastIndexOf('/');
        if (lastSlash !== -1) {
            return { from: from - (textBefore.length - lastSlash), to };
        }
        return { from, to };
    };
    ...
    return createSuggestionItems([
        {
            ...
            icon: <Text size={18} />,
            command: (props: any) => {
                const editor = props.editor || props;
                const range = getRange(editor, props.range);
